...
class SiteMatrixInterwikiResolver extends BaseInterwikiResolver {
	...
	private function siteMatrixLoader() {
		return function () {
			...
			$prefixOverrides = $this->config->get( CirrusConfigNames::InterwikiPrefixOverrides );
			$sisterProjects = [];
			$crossLanguage = [];
			$prefixesByWiki = [];
			$languageMap = [];
			...
			foreach ( $matrix->getSites() as $site ) {
				...
				if ( !in_array( $prefix, $this->config->get( CirrusConfigNames::CrossProjectSearchBlockList ) ) ) {
