...
class ConnectionTest extends CirrusIntegrationTestCase {
	...
	public static function readOnlyProvider() {
		...
		return [
			'one cluster, writable' => [
				$config + [ 'CirrusSearchClusters' => [ 'dc1' => $clusters['dc1'] ] ],
				[ 'dc1' => false ],
			],
			'one cluster, explicitly writable' => [
				$config + [
					'CirrusSearchClusters' => [ 'dc1' => $clusters['dc1'] ],
					'CirrusSearchWriteClusters' => [ 'dc1' ],
				],
				[ 'dc1' => false ],
			],
			'two clusters, one writable' => [
				$config + [
					'CirrusSearchClusters' => $clusters,
					'CirrusSearchWriteClusters' => [ 'dc1' ],
				],
				[ 'dc1' => false , 'dc2' => true ],
			],
			'two clusters, one writable, reverse order' => [
				$config + [
					'CirrusSearchClusters' => [
						'dc2' => $clusters['dc2'],
						'dc1' => $clusters['dc1'],
	 ...
	 */
	public function testReadOnly( $config, $expect ) {
		foreach ( $expect as $cluster => $isReadOnly ) {
			$conn = Connection::getPool( new HashSearchConfig( $config ), $cluster );
			$this->assertEquals( $isReadOnly, $conn->isReadOnly() );
		}
	}
}
